...
def test_grain_uniqueness(db):
    ...
    with psycopg.connect(db) as conn, conn.cursor() as cur:
        cur.execute(
            "select count(*) from ("
            "  select reservation_id, stay_date from reservations"
...
def test_expansion_pure_transform():
